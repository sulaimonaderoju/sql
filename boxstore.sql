...
**
**History:  2024-01-26
**            CREATING THE PEOPLE TABLE boxstore
**           -history at top documented
**           -a section line and section description below the previous
**           -DROP/CREATE/TRUNCATE/INSERT/SELECT people block
**           -CSV import code between the INSERT and SELECT queries
...
**			 - ENVELOPE query
**History:  2024-03-27
**            CREATING THE MANUFACTURER TABLE boxstore
**           - people meta loaded TABLE
**			 - end populate people 3 to 10002
**			 - JOIN from people to address_type and towncity>region>country
**           - people_employee TABLE
**           - DROP/CREATE/TRUNCATE/INSERT/SELECT (DCTIS)manufacturer block
**           - VIEW select
**			 - Final VIEW query
**History:  2024-04-06
...
DROP DATABASE IF EXISTS sa_0397711_boxstore;
CREATE DATABASE IF NOT EXISTS sa_0397711_boxstore
CHARSET='utf8mb4'
COLLATE='utf8mb4_unicode_ci';

USE sa_0397711_boxstore;

ALTER TABLE IF EXISTS orders DROP CONSTRAINT orders_FK_p_cus;
ALTER TABLE IF EXISTS orders DROP CONSTRAINT orders_FK_p_emp;
...
-- JOIN -- region to country (exclude active BIT field)
SELECT grg.rg_id, grg.rg_name, grg.rg_abbr, gco.co_id
...
-- JOIN -- towncity to region
SELECT gtc.tc_id, gtc.tc_name, gtc.rg_id				-- towncity columns
	 , grg.rg_id, grg.rg_name, grg.rg_abbr, grg.co_id	-- region columns
	 , gco.co_id, gco.co_name, gco.co_abbr				-- country columns
FROM   geo_towncity gtc									-- towncity columns
	   JOIN   geo_region grg ON gtc.rg_id=grg.rg_id		-- region columns
	   JOIN   geo_country gco ON grg.co_id=gco.co_id	-- country columns
;
...
-- ------------------------------------------------------------------
-- BULK LOAD insert 10000 people records from CSV file

LOAD DATA 
INFILE "D:/_data/_imports/sa_0397711_boxstore_people_10000.csv"
INTO TABLE `people`
...
-- ------------------------------------------------------------------
-- UPDATE people for my record and your record (2)
...
ALTER TABLE people 
  ...
  , ADD COLUMN tc_id         MEDIUMINT(11)      -- FK geo_towncity
  , ADD COLUMN employee      BIT NOT NULL DEFAULT 0
  , ADD COLUMN usermod      MEDIUMINT(11) NOT NULL DEFAULT 2
  , ADD COLUMN datemod DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP
  , ADD COLUMN useract      MEDIUMINT(11) DEFAULT 1
  , ADD COLUMN dateact DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP
  , ADD COLUMN active        BIT(1) NOT NULL DEFAULT 1
;

-- ------------------------------------------------------------------
-- Create 2 UPDATEs for the instructor and my record

UPDATE people 
SET    first_name='Brad' 
     , last_name='Vincelette'  
     , email_addr='brad.vincelette@boxstore.com'    
     , password=MD5('vehicle-DIAPERS_345987!') -- 9776e3ed21631ad69cfd769dedfe318d'  
     , phone_pri='204-309-7171'
     , phone_sec='204-147-1701' 
     , phone_fax='204-247-1445' 
     , addr_prefix= NULL 
     , addr='675 Oakenwald Street' 
     , addr_code='R3T 1S3'      
     , addr_info='PO Box 2471' 
...
SET    first_name='Sulaimon' 
     , last_name='Aderoju'  
     , email_addr='sulaimon.aderoju@boxstore.com'    
     , password='47eae28598b28a5b351192a2361a858e'   
     , phone_pri='204-317-7109'
     , phone_sec='204-471-1702' 
     , phone_fax='204-441-5247' 
     , addr_prefix='817'
     , addr='1744 Pembina Highway' 
     , addr_code='R3S 1T5'      
     , addr_info= NULL 
...
-- JOIN from people to address_type and towncity>region>country
SELECT p.p_id, p.first_name, p.last_name, p.email_addr, p.password
	 , p.phone_pri, p.phone_sec, p.phone_fax, p.addr_prefix, p.addr
 	 , p.addr_code, p.addr_info, p.addr_delivery, p.addr_type_id
 	 , p.tc_id, p.employee, p.usermod, p.useract, p.dateact
...
CREATE TABLE IF NOT EXISTS manufacturer (
  ...
  , addr_type_id  TINYINT(4)   -- FK geo_address_type
  , tc_id         MEDIUMINT(11)      -- FK geo_towncity
 --  , employee      BIT NOT NULL DEFAULT 0
  , usermod      MEDIUMINT(11) NOT NULL DEFAULT 2
  , datemod     DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP
  , useract      MEDIUMINT(11) DEFAULT 1
...
CREATE OR REPLACE VIEW manufacturer_people_reps
 	 ( people_id_manufacturer, manufacturer_id, manufacturer_name
 	 , manufacturer_street_address, manufacturer_po_box
 	 , manufacturer_address_type_id, manufacturer_towncity_id
 	 , manufacturer_phone_primary, manufacturer_phone_secondary
 	 , manufacturer_phone_fax
	 , manufacturer_address_prefix, manufacturer_address_code
	 , first_name, last_name, email_address, phone_primary
	 , address_type_id, address_type
 	 , towncity_id, towncity_name, towncity_region_id				
	 , region_id, region_name, region_abbrivation, region_country_id	
	 , country_id, country_name, country_abbrivation)

AS

SELECT m.p_id_man, m.man_id, m.man_name, m.addr, m.addr_info
	 , m.addr_type_id, m.tc_id, m.phone_pri, m.phone_sec, m.phone_fax
	 , m.addr_prefix, m.addr_code
...
SELECT manufacturer_name, manufacturer_street_address
	 , manufacturer_po_box, towncity_name, region_name
	 , country_name, manufacturer_phone_primary
	 , manufacturer_phone_secondary, manufacturer_phone_fax
	 , first_name, last_name, phone_primary
FROM manufacturer_people_reps
ORDER BY country_name, region_name, towncity_name 
	 , manufacturer_name
...
-- ------------------------------------------------------------------
-- JOIN 2.1.C item thru item_inventory
...
SELECT order_id, o.order_no, o.order_date
...
SELECT oi.oi_id, oi.oi_qty
	 ...
	 , i.it_id
	 , it.it_id, it.it_desc
	 , ip.ip_id, ip.item_id, ip.ip_beg, ip.ip_end, ip.item_price
--	 , AS subtotal_item
--	 , FUNC(ii_serialno) AS ii_serial_nos
FROM orders__item oi
...
FROM orders__item oi
	 ...
	 JOIN orders o ON oi.order_id=o.order_id
WHERE o.order_date BETWEEN ip.ip_beg AND IFNULL (ip.ip_end, o.order_id);
...
-- -------------------------------------------------------------------
-- First VIEW: receipt_00_header_company_info (use Boxstore Inc. only 
-- information)
...
SELECT oi.order_id
     ...
     , GROUP_CONCAT(ii.ii_serialno) AS serial_numbers
     , SUM(i.item_price) AS subtotal_item
FROM orders__item oi
     JOIN item i ON oi.item_id=i.item_id
     JOIN item_inventory ii ON oi.oi_id=ii.oi_id
     JOIN orders o ON oi.order_id=o.order_id
...
AS (
	SELECT order_id, order_date, SUM(subtotal_item) AS subtotal
...
AS (
	SELECT r20.order_id
	     ...
	     , (r20.subtotal * (t.tax_perc/100)) AS tax_amount
